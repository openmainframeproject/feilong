...
def create_config_drive(userid, network_interface_info, os_version):
    ...
    :param dict network_interface_info: Required keys:
        ...
        cidr - network CIDR
    :param str os_version: operating system version of the guestversion
    """
    userid_tmp_path = os.path.join(CONF.guest.temp_path, userid)
    if not os.path.exists(userid_tmp_path):
        os.makedirs(userid_tmp_path)
    cfg_dir = os.path.join(userid_tmp_path, 'openstack')
    ...
    if udev_settle:
        if len(net_conf_cmds) == 0:
            znetconfig = '\n'.join(('#!/bin/bash', udev_settle))
        else:
            znetconfig = '\n'.join(('#!/bin/bash', net_conf_cmds, udev_settle))
        znetconfig += '\nrm -rf /tmp/znetconfig.sh\n'
        generate_file(znetconfig, znetconfig_file)
    ...
    tar_path = os.path.join(userid_tmp_path, 'cfgdrive.tgz')
    tar = tarfile.open(tar_path, "w:gz")
    os.chdir(userid_tmp_path)
    tar.add('openstack')
    tar.close()
