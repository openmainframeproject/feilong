...
Socket Error          101     MODRC    x     The SDK server or client socket
                                             error.
...
Invalid API name      400     SVRRC    x     The SDK server received a invalid
                                             API name.

Object Not Exist      404     MODRC    x     The operated object does not
                                             exist, eg, the guest/vswitch/
                                             image/volume.

Conflict              409     MODRC    x     The status of the to-be-updated
                                             object is conflict with the
                                             update request.

Object Deleted        410     MODRC    x     The operated object has been
                                             deleted and not exist any more.
                                             This can be used for some module
                                             that support deleted=1 in DB.

Internal error        500     MODRC    x     The SDK module got unexpected
...
# 'rc' as the sdkserver or sdkclient module rc when got error, and use 'rs' as
# the socket error code.
    'socket': [{'overallRC': 101, 'rc': None},
               {},
               "SDK client or server get socket error",
               ],
# General Errors for each module, same overallRC = 300
# Guest Operation failed
    'guest': [{'overallRC': 300, 'rc': ModRCs['guest']},
...
# Network Operation failed
    'network': [{'overallRC': 300, 'rc': ModRCs['network']},
                {1: "Database operation failed, error: %(msg)s",
                 },
                "Operation on Network failed"
                ],
# Image Operation failed
    'image': [{'overallRC': 300, 'rc': ModRCs['image']},
              {1: "Database operation failed, error: %(msg)s",
               },
              "Operation on Image failed"
              ],
# Volume Operation failed
    'volume': [{'overallRC': 300, 'rc': ModRCs['volume']},
               {1: "Database operation failed, error: %(msg)s",
                },
               "Operation on Volume failed"
               ],
# Monitor Operation failed
    'monitor': [{'overallRC': 300, 'rc': ModRCs['monitor']},
                {1: "Database operation failed, error: %(msg)s",
                 },
                "Operation on Monitor failed"
                ],
# Invalid API name (Only used by sdk client/server)
# 'rc' would be set to ModRC['sdkserver'] or ModRC['sdkclient']
# 'rs' would also be set when get this error.
    'API': [{'overallRC': 400, 'rc': None},
            {},
            "Invalid API name"
            ],
# Object not exist
# Used when the operated object does not exist.
# 'rc' would be set to each module rc when raise the exception
# 'rs' is always 1
    'notExist': [{'overallRC': 404, 'rc': None},
...
# Conflict Error (The to-be-updated object status conflict)
    'conflict': [{'overallRC': 409, 'rc': None},
                 {},
...
# used.
    'internal': [{'overallRC': 500, 'rc': None},
