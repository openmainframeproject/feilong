...
def load_output(data):
    # FIXME after we upgrade python3 to py36
...
class TestzCCClient(object):
    ...
    def guest_live_migrate_vm(self, userid, zcc_userid,
                         destination, parms, operation):
        body = {"action": "live_migrate_vm",
                   "zcc_userid": zcc_userid,
...
class ZVMConnectorTestUtils(object):
    ...
    def get_image_root_disk_size(self, image_name):
        ...
        return load_output(resp.content)['output']
...
# Will use the first image as the default image to deploy guest.
    def deploy_guest(self, userid=None, cpu=1, memory=2048,
        ...
        output = load_output(resp_gcni.content)['output']
    ...
    def get_guest_power_state(self, userid):
        ...
        power_state = load_output(resp.content)['output']
        return power_state
