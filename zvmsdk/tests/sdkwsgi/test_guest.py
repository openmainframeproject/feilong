...
class GuestHandlerTestCase(unittest.TestCase):
    ...
    def _guest_deploy_with_remote_host(self, userid=None, vdev=None,
                                       image=None, transportfiles=None, remotehost=None):
        if image is None:
            image = '46a4aea3_54b6_4b1c_8a49_01f302e70c60'
        ...
        body = """{"action": "deploy",
                   ...
                   "vdev": "%s",
                   "transportfiles": "%s",
                   "remotehost": "%s"}""" % (image, vdev, transportfiles, remotehost)
    ...
    def test_guest_deploy_with_remote_host(self):
        ...
        time.sleep(10)
        try:
            resp = self._guest_deploy_with_remote_host(transportfiles=transport_file, remotehost=remote_host)
            self.assertEqual(200, resp.status_code)
            resp = self._guest_start()
            self.assertEqual(200, resp.status_code)
        except Exception as e:
