...
class VSwitchTestCase(unittest.TestCase):
    ...
    def test_vswitch_delete_update_query_not_exist(self):
        ...
        body = '{"vswitch": {"user_vlan_id": {"userid": "\
                 FVTUSER1", "vlanid":  10}}}'
        resp = self.client.api_request(url='/vswitches/RESTVSW1',
                                           method='PUT', body=body)
        self.assertEqual(404, resp.status_code)
    ...
    def test_vswitch_set_port_invalid_vlanid(self):
        ...
        try:
            ...
            body = '{"vswitch": {"user_vlan_id": {"userid": "\
                     FVTUSER1", "vlanid":  4095}}}'
            resp = self.client.api_request(url='/vswitches/RESTVSW1',
                                           method='PUT', body=body)
            self.assertEqual(500, resp.status_code)

            body = '{"vswitch": {"user_vlan_id": {"userid": "\
                     FVTUSER1", "vlanid":  "BADAWARE"}}}'
            resp = self.client.api_request(url='/vswitches/RESTVSW1',
                                           method='PUT', body=body)
            self.assertEqual(500, resp.status_code)
        except Exception:
    ...
    def test_vswitch_create_vlan_unaware(self):
        ...
        try:
            ...
            self.assertIn('FF00', vswinfo['real_devices'].keys())
        except Exception:
            raise
        finally:
            resp = self._vswitch_delete()
    ...
    def test_vswitch_create_vlan_aware(self):
        ...
        try:
            ...
            self.assertEqual('0001', vswinfo['native_vlan_id'])
