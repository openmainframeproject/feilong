...
class SDKXCATCientTestCases(SDKZVMClientTestCase):
    ...
    def test_couple_nic(self, xrequest):
        url = "/xcatws/nodes/fakezhcp/dsh?userName=" +\
                CONF.xcat.username +\
               "&password=" + CONF.xcat.password +\
               "&format=json"
        ...
        self._zvmclient._couple_nic("fakezhcp", "fakevs",
                                    "fakeuserid", "fakecdev", True, True)
    ...
    def test_uncouple_nic(self, xrequest):
        ...
        self._zvmclient._uncouple_nic("fakezhcp", "fakeuserid",
                                      "fakecdev", True, True)
        xrequest.assert_any_call("PUT", url, body1)
        xrequest.assert_any_call("PUT", url, body2)
    ...
    def test_couple_nic_to_vswitch(self, couple_nic, get_nic_settings):
        ...
        self._zvmclient.couple_nic_to_vswitch("fake_VS_name",
                                              "fake_port_name",
                                              "fakezhcp", "fake_userid",
                                              True, True)

        get_nic_settings.assert_called_with("fake_port_name", "interface")
        couple_nic.assert_called_with("fakezhcp", "fake_VS_name",
                                      "fake_userid",
                                      "fakevdev", True, True)
    ...
    def test_uncouple_nic_from_vswitch(self, uncouple_nic, get_nic_settings):
        ...
        get_nic_settings.assert_called_with("fake_port_name", "interface")
        uncouple_nic.assert_called_with("fakezhcp", "fake_userid",
                                        "fakevdev", True, True)

    @mock.patch.object(zvmclient.XCATClient, '_get_userid_from_node')
    def test_get_zhcp_userid(self, get_userid_from_node):
        self._zvmclient.get_zhcp_userid("fakezhcp")
        get_userid_from_node.assert_called_with("fakezhcp")

    @mock.patch.object(zvmclient.XCATClient, 'get_zhcp_userid')
    @mock.patch.object(zvmutils, 'xcat_request')
    def test_check_vswitch_status(self, xrequest, get_zhcp_userid):
        ...
        self._zvmclient._check_vswitch_status("fakezhcp", "fakevsw")
        get_zhcp_userid.assert_called_with("fakezhcp")
        xrequest.assert_called_with("PUT", url, body)
    ...
    def test_set_vswitch_rdev(self, xrequest, get_zhcp_userid):
        ...
        self._zvmclient._set_vswitch_rdev("fakezhcp",
                                          "fakevsw", "fakerdev")
        get_zhcp_userid.assert_called_with("fakezhcp")
        xrequest.assert_called_with("PUT", url, body)
    ...
    def test_add_vswitch(self, xrequest, get_zhcp_userid,
        ...
        self.assertRaises(exception.ZVMException,
                          ...
                          "fakezhcp", "fakename", "fakerdev",
                          '*', 1, 8, 0, 2, ["fake_vid"], 1, 1, 2, 1)
        check_vswitch_status.assert_called_with("fakezhcp", "fakename")
        xrequest.assert_called_with("PUT", url, body)
