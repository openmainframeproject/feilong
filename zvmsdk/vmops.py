...
class VMOps(object):
    ...
    def live_resize_cpus(self, userid, cpu_cnt):
        ...
        if userid.upper() not in self.guest_list():
            err_msg = "guest doesn't exist in guests database"
            LOG.error("Failed to live resize cpus of guest %s, error: %s" %
                      (userid, err_msg))
