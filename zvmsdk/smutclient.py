...
#                   "https": HTTPSBackend
    ...
    def _get_defined_cpu_addrs(self, userid):
        ...
        for ent in user_direct:
            ...
            if 'MACHINE ESA' in ent:
                max_cpus = int(ent.split()[2].strip())

        return (max_cpus, defined_addrs)

    def _get_available_cpu_addrs(self, userid, used_addrs, max_cpus):
    ...
    def resize_cpus(self, userid, count):
        ...
        if max_cpus == 0:
            LOG.error("Live resize for guest '%s' cann't be done. The maximum "
        ...
        if count > max_cpus:
            LOG.error("Live resize for guest '%s' cann't be done. The "
