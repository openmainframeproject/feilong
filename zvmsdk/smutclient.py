...
class SMUTClient(object):
    ...
    def _set_vswitch_exception(self, error, switch_name):
        if ((error.results['rc'] == 212) and (error.results['rs'] == 40)):
            ...
            raise exception.SDKConflictError(modId='network', rs=4,
                                             vsw=switch_name,
                                             msg=errmsg)
    ...
    def _create_nic_active_exception(self, error, vdev, userid):
            ...
            try:
                ...
                try:
                    ...
                    if ((results['rc'] == 404) and
                        ...
                        persist_OK = False
                if persist_OK:
                    self._create_nic_active_exception(err1, vdev, userid)
                else:
    ...
    def _delete_nic_active_exception(self, error, vdev, userid):
    ...
    def _delete_nic_inactive_exception(self, error, vdev, userid):
        if ((error.results['rc'] == 400) and (error.results['rs'] == 12)):
            obj_desc = "Guest %s" % userid
        ...
        try:
            ...
            if ((results['rc'] == 404) and
                ...
                LOG.error("Failed to delete nic %s for %s in "
                          "the guest's user direct, error: %s" %
                          (vdev, userid, emsg))
                self._delete_nic_inactive_exception(err, vdev, userid)
            ...
            try:
                ...
                if ((results['rc'] == 204) and
                    ...
                    LOG.error("Failed to delete nic %s for %s on "
                              ...
                              (vdev, userid, emsg))
                    self._delete_nic_active_exception(err, vdev, userid)
        msg = ('Delete nic device %(vdev)s for guest %(vm)s successfully'
                % {'vdev': vdev, 'vm': userid})
        LOG.info(msg)

    def _couple_active_exception(self, error, vdev, userid, vswitch):
                ...
                if ((results1 is not None) and
                    ...
                    try:
                        ...
                        if ((results2 is not None) and
                            ...
                            persist_OK = False
                    if persist_OK:
                        self._couple_active_exception(err1, vdev, userid,
                                                      vswitch_name)
                    else:
    ...
    def _uncouple_active_exception(self, error, vdev, userid):
        ...
        if active:
            ...
            try:
                ...
                if ((results is not None) and
                    ...
                    self._uncouple_active_exception(err, vdev, userid)
...
#                   "https": HTTPSBackend
    ...
    def _undedicate_nic_active_exception(self, error, vdev, userid):
    ...
    def _undedicate_nic_inactive_exception(self, error, vdev, userid):
        if ((error.results['rc'] == 400) and (error.results['rs'] == 12)):
            obj_desc = "Guest %s" % userid
            raise exception.SDKConflictError(modID='network', rs=16,
                                             userid=userid, vdev=vdev,
            ...
            try:
                ...
                if ((results['rc'] == 404) and
                    ...
                    LOG.error("Failed to undedicate nic %s for %s in "
                              "the guest's user direct, error: %s" %
                              (vdev, userid, emsg))
                self._undedicate_nic_inactive_exception(err, vdev, userid)
            def_vdev = str(hex(int(def_vdev, 16) + 1))[2:]
                ...
                try:
                    ...
                    if ((results['rc'] == 204) and
                        ...
                        LOG.error("Failed to undedicate nic %s for %s on "
                                  "the active guest system, error: %s" %
                                  (vdev, userid, emsg))
                        self._undedicate_nic_active_exception(err, vdev,
                                                              userid)
                def_vdev = str(hex(int(def_vdev, 16) + 1))[2:]
