...
class SDKAPI(object):
    ...
    def guest_config_minidisks(self, userid, disk_info):
        ...
        :param disk_info: a list contains disks info for the guest. It
               ...
               vdev for ephemeral disk. and if mntdir is not specified, zvmsdk
               will use /mnt/ephemeral0 as the mntdir.

               Here are some examples:
               [{'vdev': '0101',
               'format': 'ext3',
               'mntdir': '/mnt/ephemeral0'}]
               In this case, the zvmsdk will treat 0101 as ephemeral disk's
               vdev, and it's formatted with ext3, and will be mounted to
               /mnt/ephemeral0
               [{'format': 'ext3'},
               {'format': 'ext4'}]
               In this case, if CONF.zvm.user_root_vdev is 0100, zvmsdk will
               configure the ephemeral disk 0101, mount it to /mnt/ephemeral0
               with ext3, and configure the ephemeral disk 0102, mount it to
               /mnt/ephemeral1 with ext4.
