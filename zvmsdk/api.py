...
class SDKAPI(object):
    ...
    def guest_pre_migrate(self, userid, meta, net_set, comments):
        ...
        :param net_set: (str) the net_set of the vm, default is 0.
        ...
        if not zvmutils.check_userid_exist(userid):
            ...
            if userid in guests:
                ...
                old_comments = self._GuestDbOperator.get_comments_by_userid(
                                                                    userid)
                old_comments_dict = json.loads(old_comments)
                old_comments_dict['migrated'] = 0
                new_comments = json.dumps(old_comments_dict)
        ...
        if action.lower() == 'move':
            ...
            with zvmutils.log_and_reraise_sdkbase_error(operation):
                self._vmops.live_migrate_vm(userid, destination,
                                             parms, action)
            old_comments = self._GuestDbOperator.get_comments_by_userid(userid)
            old_comments_dict = json.loads(old_comments)
            old_comments_dict['migrated'] = 0
            new_comments = json.dumps(old_comments_dict)
            action = "update guest '%s' in database" % userid
