...
class SDKAPI(object):
    ...
    def guest_active_network_interface(self, userid, os_version):
        """ Call zvmguestconfigure to active network configuration.
        It is generally called after user adds a new network interface through
        API guest_create_network_interface which setting active True and the
        new interface has already been coupled and granted to a vswitch

        :param str userid: the user id of the guest
        :param str os_version: operating system version of the guest
        """
        action = "active the network interface for guest %s" % userid
        with zvmutils.log_and_reraise_sdkbase_error(action):
