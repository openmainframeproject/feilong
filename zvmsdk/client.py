...
class XCATClient(ZVMClient):
    ...
    def create_vm(self, userid, cpu, memory, disk_list, profile):
        ...
        body = [profile,
                'password=LBYONLY',
                'logonby=%s' % CONF.zvm.zvm_default_admin_userid,
                'cpu=%i' % cpu,
                'memory=%im' % memory,
    ...
    def add_mdisks(self, userid, disk_list):
        """Add additional disks other than root disk
        ...
        for idx, disk in enumerate(disk_list):
            vdev = self.generate_disk_vdev(idx)
    ...
    def generate_disk_vdev(self, offset):
        ...
        """
        vdev = self._generate_vdev(CONF.zvm.user_root_vdev, offset)
