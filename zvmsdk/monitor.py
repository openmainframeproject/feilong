        ...
        for uid in uid_list:
            if uid in cpumem_data:
                with zvmutils.expect_invalid_xcat_resp_data():
                    guest_cpus = int(cpumem_data[uid]['guest_cpus'])
                    used_cpu_time = cpumem_data[uid]['used_cpu_time']
                    used_cpu_time = int(used_cpu_time.partition(' ')[0]) * 1000
                cpu_data[uid] = {'guest_cpus': guest_cpus,
    ...
    def _get_inspect_data(self, type, uid_list):
        ...
        for uid in uid_list:
            ...
            if cache_data is not None:
                ...
                try:
                    if self._zvmclient.get_power_state(uid.lower()) == 'on':
                        update_needed = True
                        inspect_data = {}
                        break
                    else:
