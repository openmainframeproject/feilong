...
class MeteringCache(object):
    ...
    def set(self, ctype, index, data):
        ...
        target_cache['data'][index] = data

    def get(self, ctype, index):
        target_cache = self._get_ctype_cache(ctype)
        if(time.time() > target_cache['expiration']):
            return None
        else:
            return target_cache['data'].get(index, None)

    def delete(self, ctype, index):
        target_cache = self._get_ctype_cache(ctype)
        if index in target_cache['data']:
            del target_cache['data'][index]
