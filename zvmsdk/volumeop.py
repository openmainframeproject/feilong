...
class FCP(object):
    ...
    def _parse(self, init_info):
        ...
        if isinstance(init_info, list) and (len(init_info) == 5):
             self._dev_no = _get_dev_number_from_line(init_info[0])
             self._npiv_port = _get_wwpn_from_line(init_info[2])
             self._chpid = _get_chpid_from_line(init_info[3])
             self._physical_port = _get_wwpn_from_line(init_info[4])
...
class FCPManager(object):
    ...
    def init_fcp(self, host_stats):
        ...
        if (fcp_list is None):
            errmsg = _("because CONF.volume.fcp_list is empty, "
    ...
    def reserve_fcp(self, assigner_id):
