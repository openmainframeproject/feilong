...
def run_instance(instance_name, image_name, cpu, memory,
    ...
    nic_list = [{'nic_id': CONF.network.nic_name,
                 'mac_addr':'ff:ff:ff:ff:ff:ff'}]
    sdkapi.guest_create_port(instance_name, nic_list, ip_addr)

    # deploy image
    sdkapi.guest_deploy(instance_name, image_name, transportfiles)

    # Check network ready
    switch_info = sdkapi.get_vm_nic_switch_info(instance_name)
    if switch_info and '' not in switch_dict.values():
        for key in switch_dict:
            if not sdkapi.check_nic_coupled(instance_name, key):
