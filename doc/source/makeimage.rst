...
**NOTE**: An image deployed by z/VM Cloud Connector must match the disk type
...
z/VM Cloud Connector manage the deployed vm via IUCV channel. IUCV service
...
In Openstack environment, if customer want to spawn a vm by z/VM Cloud Connector,
the image for deployment should be uploaded into glance firstly. Openstack nova
zVM driver will then import the image to z/VM Cloud Connector when the image is
