...
class RESTClient(object):
    ...
    def _get_url_body_headers(self, api_name, *args, **kwargs):
        ...
        if api_name in ['image_upload']:
            headers['Content-Type'] = 'application/octet-stream'

        if api_name == 'image_upload':
            headers.update(args[count_params_in_path + 1])
    ...
    def call(self, api_name, *args, **kwargs):
        try:
            ...
            if api_name == 'image_download' and resp.status_code == 200:
                # please note that we need to handle the case if
                # Download meet exception in wsgi,eg can't find image etc.
                # In this case it will return the json stream. 
                if (resp.headers.get('Content-Type') is not 
                    'application/octet-stream'):
                    # throw out exception
                     LOG.error("For image download the content type should "
                               "be octet-stream")
                ...
                if checksum is not None:
                   self.integrity_iter(body_iter, checksum)

                target_file = DATABASE[api_name]['params_path']
                self.save_image(body_iter, target_file)
                results.updata({'overallRC': 0, 'modID': 40, 'rc': 0,
                                'output': '', 'rs': 0, 'errmsg': ''})
    ...
    def integrity_iter(self, iter, checksum):
