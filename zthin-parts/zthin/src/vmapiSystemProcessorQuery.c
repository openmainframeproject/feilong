...
#define PARSER_TABLE_NAME     System_Processor_Query_Layout

int smSystem_Processor_Query(struct _vmApiInternalContext* vmapiContextP,
        char * userid, int passwordLength, char * password,
        char * targetIdentifier, vmApiSystemProcessorQueryOutput** outData) {
    const char* const functionName = "System_Processor_Query";
    int tempSize;
    char* cursor;
    int rc = 0;
