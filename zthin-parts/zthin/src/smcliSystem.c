...
int systemRDRFileManage(int argC, char* argV[], struct _vmApiInternalContext* vmapiContextP) {
        ...
        switch (option) {
            ...
            case 'h':
                ...
                printf("NAME\n"
                    ...
                    "                    TRANSFER - Transfer the specified rdr files to the authenticated user virtual machine.\n"

                    "    -d    The user id to which the rdr file will be transferred, it's optional, only needed when action\n"
    ...
    if ( action == 'TRANSFER' && !destIdentifier ) {
        DOES_CALLER_WANT_RC_HEADER_SYNTAX_ERROR(vmapiContextP);
