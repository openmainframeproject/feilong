...
import generalUtils
from generalUtils import parseCmdline, cvtToCyl
import math
import sys
import subprocess
...
posOpsList = {
                ...
                'AEMOD'         : [
                                    [ 'Activation Engine Modification Script', 'aeScript', True, 2 ],
                                  ],
                'IPL'           : [
...
def add3390( reqHandle ):
    ...
    reqHandle.printLn( "N",  "China team: Do we need to support 'autog' as the virtual address?" )
    ...
    reqHandle.printSysLog( "Exit changeVM.add3390, rc: " + str( results['overallRC'] ) )
...
def add9336( reqHandle ):
    ...
    results, blocks = generalUtils.cvtToBlocks( reqHandle, reqHandle.parms['diskSize'] )
    if results['overallRC'] != 0 :
        # message already sent.  Only need to update the final results.
        reqHandle.updateResults( results )
    ...
    reqHandle.printSysLog( "Exit changeVM.add9336, rc: " + str( results['overallRC'] ) )
    return results['overallRC']
...
def addLOADDEV( reqHandle ):
    ...
    Input:
       Request Handle with the following properties:
          ...
          parms['lun']        - One to eight-byte logical unit number of the FCP-I/O device.
          parms['scpData']    - Designates information to be passed to the program is
                                loaded during guest IPL.
...
def doIt( reqHandle ):
    ...
    if 'showParms' in reqHandle.parms and reqHandle.parms['showParms'] == True :
        ...
        reqHandle.printLn( "N", "  Routine: changeVM." + str( subfuncHandler[reqHandle.subfunction][0] ) + "( reqHandle )" )
...
def showInvLines( reqHandle ) :
    ...
    reqHandle.printLn( "N", "  python " + reqHandle.cmdName + " ChangeVM <userid> add3390 <diskPool> <vAddr>" )
    reqHandle.printLn( "N", "                    <diskSize3390> mode <mode> readpw <read_pw>" )
    reqHandle.printLn( "N", "                    writepw <write_pw> multipw <multi_pw> filesystem <fsType>" )
    reqHandle.printLn( "N", "  python " + reqHandle.cmdName + " ChangeVM <userid> add9336 <diskPool> <vAddr>" )
    reqHandle.printLn( "N", "                    <diskSize9336> mode <mode> readpw <read_pw>" )
    reqHandle.printLn( "N", "                    writepw <write_pw> multipw <multi_pw> filesystem <fsType>" )
    reqHandle.printLn( "N", "  python " + reqHandle.cmdName + " ChangeVM <userid> aemod <aeScript> invparms <invParms>" )
    reqHandle.printLn( "N", "  python " + reqHandle.cmdName + " ChangeVM <userid> IPL <addrOrNSS> loadparms <loadParms>" )
    reqHandle.printLn( "N", "                    parms <parmString>" )
    reqHandle.printLn( "N", "  python " + reqHandle.cmdName + " ChangeVM <userid> LOADDEV wwpn <wwpn> lun <lun>" )
    reqHandle.printLn( "N", "                    scphexdata <scp_hex> scpdata <scp_data>" )
    reqHandle.printLn( "N", "  python " + reqHandle.cmdName + " ChangeVM <userid> punchFile <file> class <class>" )
    reqHandle.printLn( "N", "  python " + reqHandle.cmdName + " ChangeVM <userid> purgeRDR" )
    reqHandle.printLn( "N", "  python " + reqHandle.cmdName + " ChangeVM help" )
    reqHandle.printLn( "N", "  python " + reqHandle.cmdName + " ChangeVM version" )
    return
...
def showOperandLines( reqHandle ) :
    ...
    reqHandle.printLn( "N", "      add3390       - Add a 3390 (ECKD) disk to a virtual machine's directory" )
    reqHandle.printLn( "N", "                      entry." )
    reqHandle.printLn( "N", "      add9336       - Add a 9336 (FBA) disk to virtual machine's directory" )
    reqHandle.printLn( "N", "                      entry." )
    reqHandle.printLn( "N", "      aemod         - Sends an activation engine script to the managed virtual" )
    reqHandle.printLn( "N", "                      machine." )
    reqHandle.printLn( "N", "      help          - Displays this help information." )
    reqHandle.printLn( "N", "      ipl           - Sets the IPL statement in the virtual machine's" )
    reqHandle.printLn( "N", "                      directory entry." )
    reqHandle.printLn( "N", "      loaddev       - Sets the LOADDEV statement in the virtual machine's" )
    reqHandle.printLn( "N", "                      directory entry." )
    reqHandle.printLn( "N", "      punchfile     - Punch a file to a virtual reader of the specified" )
    reqHandle.printLn( "N", "                      virtual machine." )
    reqHandle.printLn( "N", "      purgerdr      - Purges the reader belonging to the virtual machine." )
