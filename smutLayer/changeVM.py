...
subfuncHandler = {
    ...
    'AEMOD':   ['addAEMOD', lambda rh: add9336(rh)],
...
def showOperandLines(rh):
    ...
    if rh.subfunction != '':
        ...
        rh.printLn("N", "      <addrOrNSS>           - Specifies the virtual address or NSS name")
        rh.printLn("N", "                              to IPL.")
        rh.printLn("N", "      <aeScript>            - aeScript is the fully qualified file")
        rh.printLn("N", "                              specification of the script to be sent")
        rh.printLn("N", "      class <class>         - The class is optional and specifies the spool")
        rh.printLn("N", "                              class for the reader file.")
        rh.printLn("N", "      <diskPool>            - Specifies the directory manager disk pool to")
        rh.printLn("N", "                              use to obtain the disk.")
        rh.printLn("N", "      <diskSize3390>        - Specifies the size of the ECKD minidisk.  ")
        rh.printLn("N", "      <diskSize9336>        - Specifies the size of the FBA type minidisk.")
        rh.printLn("N", "      <file>                - File to punch to the target system.")
        rh.printLn("N", "      filesystem <fsType>   - Specifies type of filesystem to be created on")
        rh.printLn("N", "                              the minidisk.")
        rh.printLn("N", "      invparms <invParms>   - Specifies the parameters to be specified in the")
        rh.printLn("N", "                              invocation script to call the aeScript.")
        rh.printLn("N", "      loadparms <loadParms> - Specifies a 1 to 8-character load parameter that")
        rh.printLn("N", "                              is used by the IPL'd system.")
        rh.printLn("N", "      lun <lun>             - One to eight-byte logical unit number of the")
        rh.printLn("N", "                              FCP-I/O device.")
        rh.printLn("N", "      mode <mode>           - Specifies the access mode for the minidisk.")
        rh.printLn("N", "      multipw <multi_pw>    - Specifies the password that allows sharing the")
        rh.printLn("N", "                              minidisk in multiple-write mode.")
        rh.printLn("N", "      parms <parmString>    - Specifies a parameter string to pass to the")
        rh.printLn("N", "                              virtual machine in general-purpose registers at")
        rh.printLn("N", "                              user's the completion of the IPL.")
        rh.printLn("N", "      readpw <read_pw>      - Specifies the password that allows sharing the")
        rh.printLn("N", "                              minidisk in read mode.")
        rh.printLn("N", "      scpdata <scp_data>    - Provides the SCP data information.")
        rh.printLn("N", "      scphexdata <scp_hex>  - Provides the SCP data information as hexadecimal")
        rh.printLn("N", "                              representation of UTF-8 data.")
        rh.printLn("N", "      <userid>              - Userid of the target virtual machine.")
        rh.printLn("N", "      <vAddr>               - Virtual address of the device.")
        rh.printLn("N", "      writepw <write_pw>    - Specifies is the password that allows sharing")
        rh.printLn("N", "                              the minidisk in write mode.")
        rh.printLn("N", "      wwpn <wwpn>           - The world-wide port number.")
