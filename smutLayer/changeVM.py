...
def add9336(rh):
    ...
    if results['overallRC'] == 0:
        results = vmUtils.isLoggedOn(rh, rh.userid)
        if (results['overallRC'] == 0 and results['rs'] == 0):
            # Add the disk to the active configuration.
            cmd = ["smcli",
                "Image_Disk_Create",
...
def removeDisk(rh):
    ...
    if results['overallRC'] == 0:
        if results['rs'] == 0:
            ...
            disableEnableDisk(rh, rh.userid, parms['vaddr'], '-d')
            if results['overallRC'] != 0:
                printSysLog("Command failed with rc: "+ str(results['rc']) +
    ...
    if results['overallRC'] == 0 and loggedOn:
        strCmd = ["/sbin/vmcp detach "+ parms['vaddr']]
