...
keyOpsList = {
    'DIRECTORY': {
        ...
        '--maxCPU': ['maxCPU', 1, 1],
        '--setStandbyMem': ['setStandbyMem', 0, 0],
...
def createVM(rh):
    ...
    if 'setStandbyMem' in rh.parms and (priMem != maxMem):
        standbySize = getStandbyMemSize(rh, priMem, maxMem)
        ...
        if standbySize != '0M':
            ...
            dirLines.append("COMMAND DEF STOR STANDBY %s" % standbySize)
            dirLines.append("COMMAND SET RUN ON")
...
def showOperandLines(rh):
    ...
    if rh.subfunction != '':
        ...
        rh.printLn("N", "      --setStandbyMem       - " +
                   "Set the additional memory space (maxMemSize - priMemSize)")
        rh.printLn("N", "                              " +
                   "as standby memory of the virtual machine.")
...
def getStandbyMemSize(rh, mem, maxMem):
    rh.printSysLog("Enter makeVM.getStandbyMemSize")
    ...
    rh.printSysLog("Exit makeVM.getStandbyMemSize, rc: " +
        str(rh.results['overallRC']))
