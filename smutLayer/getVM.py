...
def extract_fcp_data(raw_data):
    """
    extract data from smcli System_WWPN_Query output.
    Input:
        raw data returned from smcli
    Output:
        ata extracted would be like:
    [{'status': 'Free', 'fcp_dev_no': '1D2F',
      'physical_wwpn': 'C05076E9928051D1', 'channel_path_id': '8B',
      'npiv_wwpn': 'NONE'},
     {'status': 'Free', 'fcp_dev_no': '1D29',
      'physical_wwpn': 'C05076E9928051D1', 'channel_path_id': '8B',
      'npiv_wwpn': 'NONE]
    """
    ...
    for i in range(0, len(data), 5):
        item = {}
        item['fcp_dev_no'] = data[i].split(':')[-1].strip()
        item['status'] = data[i + 1].split(':')[-1].strip()
        item['npiv_wwpn'] = data[i + 2].split(':')[-1].strip()
        item['channel_path_id'] = data[i + 3].split(':')[-1].strip()
        item['physical_wwpn'] = data[i + 4].split(':')[-1].strip()
        results.append(item)
...
def fcpinfo(rh):
    ...
    if results['overallRC'] == 0:
        ...
        ret = extract_fcp_data(results['response'])
