...
def cancelMigrate(rh):
    ...
    if results['overallRC'] != 0:
        strCmd = ' '.join(cmd)
        rh.printLn("ES",  "Command failed: '" + strCmd + "', out: '" + results['response'] + 
                                  "', rc: " + str(results['overallRC']))
        rh.updateResults(results)

    rh.printSysLog("Exit migrateVM.cancelMigrate, rc: " + str(results['overallRC']))
...
def doIt(rh):
    ...
    if 'showParms' in rh.parms and rh.parms['showParms'] is True:
        ...
        rh.printLn("N", "  Routine: migrateVM." + str(subfuncHandler[rh.subfunction][0]) + "(rh)")
...
def getStatus(rh):
    ...
    rh.printSysLog("Exit migrateVM.getStatus, rc: " + str(results['overallRC']))
    return results['overallRC']
...
def modifyMigrate(rh):
    ...
    rh.printSysLog("Exit migrateVM.modifyMigrate, rc: " + str(results['overallRC']))
    return results['overallRC']
...
def showOperandLines(rh):
    ...
    rh.printLn("N", "      --immediate            - " +
        "causes the VMRELOCATE command to do one early pass")
    rh.printLn("N", "                               " +
        "through virtual machine storage and then go")
    rh.printLn("N", "                               " +
    ...
    rh.printLn("N", "      --maxtotal <maxVal>    - " +
        "indicates the maximum total time (in seconds) for")
    rh.printLn("N", "                               relocation to complete.")
