        ...
        if results['overallRC'] == 0:
            if len(totals) == 0:
                ...
                for poolName in sorted(totals):
                    ...
                    rh.printLn("N", poolName + " Total: " + generalUtils.cvtToMag(rh, total))
                    rh.printLn("N", poolName + " Used: " + generalUtils.cvtToMag(rh, totals[poolName]["3"]))
                    rh.printLn("N", poolName + " Free: " + generalUtils.cvtToMag(rh, totals[poolName]["2"]))

    rh.printSysLog("Exit getHost.getDiskPoolSpace, rc: " + str(rh.results['overallRC']))
    return rh.results['overallRC']
...
def showInvLines(rh):
    ...
    if rh.subfunction != '':
        rh.printLn("N",  "Usage:")
    rh.printLn("N", "  python " + rh.cmdName + " GetHost <userid> diskpoolnames")
    rh.printLn("N", "  python " + rh.cmdName + " GetHost <userid> diskpoolspace <poolName>")
...
def showOperandLines(rh):
    ...
    rh.printLn("N", "      diskpoolnames - Returns the names of the directory manager disk pools.")
    rh.printLn("N", "      diskpoolspace - Returns disk pool size information.")
    rh.printLn("N", "      fcpdevices    - Lists the FCP device channels that are active, free, or")
    rh.printLn("N", "                      offline.")
    rh.printLn("N", "      general       - Returns the general information related to the z/VM")
