...
from ReqHandle import *
from subprocess import CalledProcessError, check_output

"""
The try/except section will import local overrides for the subs and testSets structures
if the exist.  This allows for extensions to the defined tests without modifying 
this file.
"""
try :
...
"""
subs =  {
            '<<<safeID>>>'        : 'someid',          # An existing userid that can be started and stopped
            '<<<unsafeID1>>>'     : 'TSTUSER1',        # A userid that gets created and destroyed
            '<<<migrID>>>'        : 'someid',          # An existing userid that can be migrated
            '<<<unmigrID>>>'      : 'unmgr',           # An existing userid that cannot be migrated
            '<<<migrDest>>>'      : 'zvmhost',         # A z/VM host for migration into it
            '<<<pw>>>'            : 'SMAPI',           # password
            '<<<vmSize>>>'        : '2G',              # Virtual machine size
            '<<<pool3390>>>'      : 'POOL1',           # 3390 disk pool (keep this in uppercase for smutTest ease of use)
            '<<<size3390>>>'      : '1100',            # Size of a 3390 for system deploys
            '<<<pool9336>>>'      : 'POOL4',           # 9336 disk pool (keep this in uppercase for smutTest ease of use)
            '<<<setupDisk>>>'     : '/opt/xcat/share/xcat/scripts/setupDisk',  # SetupDisk location
            '<<<SimpleCfgFile>>>' : '/install/zvm/POC/testImages/cfgdrive.tar',    # Simple tar file for the config drive
            '<<<simpleImage>>>'   : '/install/zvm/POC/testImages/rhel67eckd_small_225M.img',          # Small image file
            '<<<unpackScript>>>'  : '/opt/zhcp/bin/unpackdiskimage',    # Location of unpackdiskimage
        }
...
This includes the following keys:
   ...
   overallRC   - A single return code or a list of return codes to compare
                 ...
                 If the test returns an overallRC value that matches one of the
                 specified valuess then it has passed the overallRC check.
   rc          - A single return code or a list of return codes.
                 If the test returns a return code that matches one of the
                 specified return codes then it has passed the return code check.
   rs          - A single return code or a list of return codes.
                 If the test returns a return code that matches one of the
                 specified return codes then it has passed the return code check.

Note: A test must pass all specified tests (e.g. output, rc, etc.) in order for
      the test to pass.
"""
...
deployTests =  [
                {
                    ...
                    'request':      "MakeVM <<<unsafeID1>>> directory <<<pw>>> <<<vmSize>>> g ipl 100 " +
                                    "profile OSDFLT",
                    'out':          "",
                    'overallRC':    [0],
                },
                {
                    ...
                    'request':      "ChangeVM <<<unsafeID1>>> add3390 <<<pool3390>>> 100 <<<size3390>>>",
                    'out':          "",
                    'overallRC':    [0],
                },
                {
                ...
                },
                {
                    ...
                    'request':      "SHELL_TEST <<<unpackScript>>> <<<unsafeID1>>> 100 <<<simpleImage>>>",
                    'out':          "",
                    'overallRC':    [0],
                },
                {
...
migrateTests = [
                {
                    'description':  "Get status for a specific userid that cannot be migrated.",
                ...
                {
                    'description':  "Get all migration status for a host with no active migrations.",
                ...
                {
                    'description':  "Get incoming migration status for a host with no active migrations.",
                ...
                {
                    'description':  "Get outgoing migration status for a host with no active migrations.",
                ...
                },
                {
                    ...
                    'request':      "migrateVM <<<unmigrID>>> test destination <<<migrDest>>>",
                    'overallRC':    [1],
                    'rc':           [4],
                    'rs':           [3000],
                },
...
powerTests   = [
                ...
                {
                    'description':  "Isreachable of a paused system should be unreachable.",
                    'request':      "PowerVM <<<safeID>>> isreachable",
                    'out':          "'<<<safeID>>>: unreachable'",
                    'overallRC':    [0],
                    'rs':           [0]
                ...
                {
                    'description':  "Isreachable of an unpaused system should be reachable.",
                    'request':      "PowerVM <<<safeID>>> isreachable",
                    'out':          "'<<<safeID>>>: reachable'",
                    'overallRC':    [0],
                    'rs':           [1]
...
shellTests =   [
                {
                    'description':  "Issue a successful non-Test 1-line output related SHELL function.",
                    'request':      "SHELL echo \'Hurray!\'",
                    'overallRC':    [0],
                },
                {
                    'description':  "Issue a successful non-Test multi-line output related SHELL function.",
                    'request':      "SHELL cat /etc/hosts",
                    'overallRC':    [0],
                },
                {
                    'description':  "Issue an unsuccessful non-Test related SHELL function.",
                    'request':      "SHELL bob",
                    'overallRC':    [127],
                },
                {
...
def runTest( smut, test ) :
    ...
    if test['request'][0:10] != 'SHELL_TEST' :
        #results = smut.request( test['request'] )
        ...
        results = {
            'overallRC'    : 0,   # Overall return code for the function
    ...
    if 'out' not in test.keys() :
        ...
        if len( results['response'] ) == 0 :
            ...
            try :
                """print( 'echo ' + ''.join(results['response']) + '|' +
                                                'grep ' + ''.join(test['out']) )"""
                junk = subprocess.check_output( '/bin/echo ' + ''.join(results['response']) + 
                                                '|grep ' + ''.join(test['out']),
                                                stderr=subprocess.STDOUT,
                                                shell=True )
                if junk != '' :
        ...
        respScore = 1       #  No responses listed as expected, treat it as a match
            ...
            if 'overallRC' in test :
                for testRC in test['overallRC'] :
                    if shellRC == testRC :
                        ...
                        break
            if shellOk == 0 :
                print( "***Warning*** A non test related shell function returned rc: " + str( shellRC ) +
                        " out: " + ''.join( out ) )
        else :
            localCnt += 1
            print("")
...
if len( sys.argv ) > 1 and sys.argv[1].upper() == '--LISTAREAS' :
    ...
    for key in testSets :
        for test in testSets[key][1] :
            test['request'] = pattern.sub(lambda m: subs[re.escape(m.group(0))], test['request'] )
            if 'out' in test :
                test['out'] = pattern.sub(lambda m: subs[re.escape(m.group(0))], test['out'] )
