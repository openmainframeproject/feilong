...
subfuncHandler = {
                    ...
                    'SOFTOFF'       : [ 'softoff', lambda reqHandle: softoff( reqHandle ) ],
...
# While the 'showparms' operand is just the keyword and has no additional value portion.
keyOpsList = { 
            'HELP'       : {},
            'ISREACHABLE'   : { 'showparms'   : [ 'showParms', 0, 0 ], },
            'OFF'           : { 'maxwait'     : [ 'maxWait', 1, 1 ],
                                'poll'        : [ 'poll', 1, 1 ],
                                'showparms'   : [ 'showParms', 0, 0 ],
                                'wait'        : [ 'wait', 0, 0 ],
                              },
            'ON'            : { 'state'       : [ 'desiredState', 1, 2 ],
                                'maxwait'     : [ 'maxWait', 1, 1 ],
                                'poll'        : [ 'poll', 1, 1 ],
                                'showparms'   : [ 'showParms', 0, 0 ],
            ...
            'REBOOT'        : { 'maxwait'     : [ 'maxWait', 1, 1 ],
                                'poll'        : [ 'poll', 1, 1 ],
                                'showparms'   : [ 'showParms', 0, 0 ],
                                'wait'        : [ 'wait', 0, 0 ],
                              },
            'RESET'         : { 'state'       : [ 'desiredState', 1, 2 ],
                                'maxwait'     : [ 'maxWait', 1, 1 ],
                                'poll'        : [ 'poll', 1, 1 ],
                                'showparms'   : [ 'showParms', 0, 0 ],
                                'state'       : [ 'desiredState', 1, 2 ],
            ...
            'SOFTOFF'       : { 'maxwait'     : [ 'maxWait', 1, 1 ],
                                'poll'        : [ 'poll', 1, 1 ],
                                'showparms'   : [ 'showParms', 0, 0 ],
                                'wait'        : [ 'wait', 0, 0 ],
                              },
            'STATUS'        : { 'showparms'   : [ 'showParms', 0, 0 ], },
            'UNPAUSE'       : { 'showparms'   : [ 'showParms', 0, 0 ], },
            'VERSION'       : {},
            'WAIT'          : { 'maxwait'     : [ 'maxWait', 1, 1 ],
                                'poll'        : [ 'poll', 1, 1 ],
                                'showparms'   : [ 'showParms', 0, 0 ],
                                'state'       : [ 'desiredState', 1, 2 ],
                              },
...
def activate( reqHandle ):
    ...
    Input:
       Request Handle with the following properties:
          ...
          parms['desiredState']   - Desired state. Optional, unless 'maxQueries' is specified.
          parms['maxQueries']     - Maximum number of queries to issue. Optional.
    ...
    if results['overallRC'] == 0 and 'maxQueries' in reqHandle.parms :
        if reqHandle.parms['desiredState'] == 'up' :
            ...
            results = waitForVMState( reqHandle, 
                          reqHandle.userid,
                          reqHandle.parms['desiredState'],
                          maxQueries=reqHandle.parms['maxQueries'],
                          sleepSecs=reqHandle.parms['poll'] )
        if results['overallRC'] == 0 :
            reqHandle.printLn( "N", "Userid '" + reqHandle.userid + " is in the desired state: " +
                               reqHandle.parms['desiredState'] )

    reqHandle.printSysLog( "Exit powerVM.activate, rc: " + str( results['overallRC'] ) )
...
def checkIsReachable( reqHandle ):
    ...
    reqHandle.printSysLog( "Enter powerVM.checkIsReachable, userid: " + reqHandle.userid )
...
def deactivate( reqHandle ):
    ...
    reqHandle.printSysLog( "Enter powerVM.deactivate, userid: " + reqHandle.userid )
    ...
    if results['overallRC'] == 0 :
        ...
        # Tolerable error.  Machine is already in or going into the state we want it to enter.
    ...
    if results['overallRC'] == 0 and 'maxQueries' in reqHandle.parms :
        results = waitForVMState( reqHandle, 
                                  ...
                                  sleepSecs=reqHandle.parms['poll'] )
        if results['overallRC'] == 0 :
            reqHandle.printLn( "N", "Userid '" + reqHandle.userid + " is in the desired state: off" )
        else :
            reqHandle.updateResults( results )

    reqHandle.printSysLog( "Exit powerVM.deactivate, rc: " + str( results['overallRC'] ) )
    return results['overallRC']
...
def doIt( reqHandle ):
    ...
    if 'showParms' in reqHandle.parms and reqHandle.parms['showParms'] == True :
        ...
        reqHandle.printLn( "N", "  Routine: powerVM." + str( subfuncHandler[reqHandle.subfunction][0] ) + "( reqHandle )" )
...
def getStatus( reqHandle ):
    ...
    reqHandle.printSysLog( "Enter powerVM.getStatus, userid: " + reqHandle.userid )
    ...
    try :
        ...
        out = e.output
        cmdRC = e.returncode
        reqHandle.printLn( "ES",  "Command failed: '" + cmd + "', rc: " + str( cmdRC ) +
                            " out: " + out )
    ...
    reqHandle.printSysLog( "Exit powerVM.getStatus, rc: " + str( reqHandle.results['overallRC'] ) )
...
def parseCmdline( reqHandle ):
    ...
    if reqHandle.totalParms >= 2 :
        ...
        reqHandle.printSysLog( "Exit powerVM.parseCmdLine, rc: " + reqHandle.results['overallRC'] )
        return reqHandle.results['overallRC']
    ...
    if reqHandle.results['overallRC'] == 0 :
        if reqHandle.subfunction == 'WAIT' :
            ...
            if reqHandle.parms['desiredState'] not in [ 'down', 'off', 'on', 'up' ] :
                reqHandle.printLn( "ES", "The desired state was not 'down', 'off', 'on' or 'up': " +
                    reqHandle.parms['desiredState'] + "." )
                reqHandle.updateResults( { 'overallRC': 4 } )
    ...
    if reqHandle.results['overallRC'] == 0 and waiting == 1 :
        ...
        if reqHandle.results['overallRC'] == 0 :
            ...
            reqHandle.parms['maxQueries'] = ( reqHandle.parms['maxWait'] + reqHandle.parms['poll'] - 1 ) / reqHandle.parms['poll']

    reqHandle.printSysLog( "Exit powerVM.parseCmdLine, rc: " + str( reqHandle.results['overallRC'] ) )
    return reqHandle.results['overallRC']
...
def reboot( reqHandle ):
    ...
    results = execCmdThruIUCV( reqHandle, reqHandle.userid, strCmd )
    if results['overallRC'] != 0 :
        reqHandle.printLn( "ES",  "Command: '" + strCmd + "' failed. ', out: '" + results['response'] + 
                          "', rc: " + str( results['overallRC'] ) )

    if results['overallRC'] == 0 :
        # Wait for the OS to go down
        results = waitForOSState( reqHandle, reqHandle.userid, "down", maxQueries=30, sleepSecs=10 )
        if results['overallRC'] == 0 :
            reqHandle.printLn( "N", "Userid '" + reqHandle.userid + " is in the interim state: down" )
    ...
    if results['overallRC'] == 0 and 'maxQueries' in reqHandle.parms :
        results = waitForOSState( reqHandle, 
                                  ...
                                  sleepSecs=reqHandle.parms['poll'] )
        if results['overallRC'] == 0 :
            reqHandle.printLn( "N", "Userid '" + reqHandle.userid + " is in the desired state: up" )
        else :
            reqHandle.updateResults( results )

    reqHandle.printSysLog( "Exit powerVM.reboot, rc: " + str( results['overallRC'] ) )
    return results['overallRC']
...
def reset( reqHandle ):
    ...
    if results['overallRC'] != 0 :
        if results['rc'] == 200 and results['rs'] == 12 :
            ...
            strCmd = ' '.join( cmd )
            reqHandle.printLn( "ES",  "Command failed: '" + strCmd + "', out: '" + reqHandle.results['response'] + 
                                      "', rc: " + str( reqHandle.results['overallRC'] ) )
            reqHandle.updateResults( results )

    # Wait for the logoff to complete
    if results['overallRC'] == 0 :
        results = waitForVMState( reqHandle, reqHandle.userid, "off", maxQueries=30, sleepSecs=10 )
    ...
    if results['overallRC'] == 0 and 'maxQueries' in reqHandle.parms :
        if reqHandle.parms['desiredState'] == 'up' :
            ...
            results = waitForVMState( reqHandle, 
                          ...
                          sleepSecs=reqHandle.parms['poll'] )
        if results['overallRC'] == 0 :
            reqHandle.printLn( "N", "Userid '" + reqHandle.userid + " is in the desired state: " + reqHandle.parms['desiredState'] )
        else :
            reqHandle.updateResults( results )

    reqHandle.printSysLog( "Exit powerVM.reset, rc: " + str( results['overallRC'] ) )
    return results['overallRC']
...
def showInvLines( reqHandle ) :
    ...
    reqHandle.printLn( "N", "  python " + reqHandle.cmdName + " PowerVM <userid>" )
    reqHandle.printLn( "N", "                    [ isreachable | pause | status | unpause ]" )
    reqHandle.printLn( "N", "  python " + reqHandle.cmdName + " PowerVM <userid>" )
    reqHandle.printLn( "N", "                    [ on | reset ] wait state [ on | up ] maxwait <secs>" )
    reqHandle.printLn( "N", "                    poll <secs>" )
    reqHandle.printLn( "N", "  python " + reqHandle.cmdName + " PowerVM <userid>" )
    reqHandle.printLn( "N", "                    [ off | reboot | softoff ] wait maxwait <secs> poll <secs>" )
    reqHandle.printLn( "N", "  python " + reqHandle.cmdName + " PowerVM <userid> wait" )
    reqHandle.printLn( "N", "                    state [ down | on | off | up ] maxwait <secs> poll <secs>" )
...
def showOperandLines( reqHandle ) :
    ...
    reqHandle.printLn( "N", "      reboot      - Issue a shutdown command to reboot the OS in a virtual" )
    reqHandle.printLn( "N", "                    machine" )
    reqHandle.printLn( "N", "      reset       - Power a virtual machine off and then back on" )
    reqHandle.printLn( "N", "      softoff     - Issue a shutdown command to shutdown the OS in a virtual" )
    reqHandle.printLn( "N", "                    machine and then log the virtual machine off z/VM." )
    ...
    reqHandle.printLn( "N", "      wait        - Wait for the virtual machine to go into the specified" )
    reqHandle.printLn( "N", "                    state of either:" )
    reqHandle.printLn( "N", "                       down: virtual machine's OS is not reachable with IUCV" )
    reqHandle.printLn( "N", "                       off: virtual machine is logged off" )
    reqHandle.printLn( "N", "                       on: virtual machine is logged on" )
    reqHandle.printLn( "N", "                       up: virtual machine's OS is reachable with IUCV" )
    ...
    reqHandle.printLn( "N", "      maxwait <secs> - Maximum time in seconds to wait" )
    reqHandle.printLn( "N", "      poll <secs> - Seconds to wait between polling" )

    return


def softDeactivate( reqHandle ):
    """ Deactivate a virtual machine by first shutting down Linux and then logging it off.
    ...
    reqHandle.printSysLog( "Enter powerVM.softDeactivate, userid: " + reqHandle.userid )
    ...
    if results['overallRC'] != 0 :
        # Tell z/VM to log off the system after we give it a 15 second head start.
        time.sleep( 15 )
        cmd = [ "smcli",
                "Image_Deactivate",
                "-T", reqHandle.userid ]
    ...
    reqHandle.printSysLog( "Exit powerVM.softDeactivate, rc: " + str( results['overallRC'] ) )
    return results['overallRC']
...
def unpause( reqHandle ):
    ...
    reqHandle.printSysLog( "Exit powerVM.unpause, rc: " + str( results['overallRC'] ) )
    return results['overallRC']
...
def wait( reqHandle ):
    ...
    if reqHandle.parms['desiredState'] == 'off' or reqHandle.parms['desiredState'] == 'on' :
        results = waitForVMState(   reqHandle, 
                                    reqHandle.userid,
                                    reqHandle.parms['desiredState'],
                                    maxQueries=reqHandle.parms['maxQueries'],
