...
def disableEnableDisk(rh, userid, vaddr, option):
    ...
    rh.printSysLog("Exit vmUtils.disableEnableDisk, rc: " + str(results['overallRC']))
    return results
...
def installFS(rh, vaddr, mode, fileSystem):
    ...
    rh.printSysLog("Enter vmUtils.installFS, userid: " + userid +
    ...
    try:
        cmd = ["./linkdiskandbringonline", userid, vaddr, mode]
    ...
    if diskAccessed:
        # Give up the disk.
        try:
            cmd = ["./offlinediskanddetach", userid, vaddr]
            out = subprocess.check_output(cmd, close_fds=True)
        except CalledProcessError as e:
            out = e.output
            results['overallRC'] = 99
...
def isLoggedOn(rh, userid):
    ...
    try:
        out = subprocess.check_output(cmd, close_fds=True, stderr=subprocess.STDOUT)
    except CalledProcessError as e:
